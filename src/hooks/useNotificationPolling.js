...
import { fetchNotificationUnreadCount, fetchNotificationSummaryByUser } from '../store/slices/notificationsSlice';

// Two independent client timers per logged-in session (spec: "Live" = polling, not push):
//   • unread-count     every ~18s → bell badge
//   • summary-by-user  every ~45s → dropdown per-user list
...
export default function useNotificationPolling() {
  ...
  useEffect(() => {
    ...
    const pollSummary = () => dispatch(fetchNotificationSummaryByUser());
